import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:loading_animation_widget/loading_animation_widget.dart';
import 'package:mindteck_iot/models/site/site_data.dart';
import 'package:mindteck_iot/provider/api_data_provider.dart';
import 'package:mindteck_iot/widgets/error_screen.dart';
import 'package:mindteck_iot/widgets/no_data_screen.dart';
import 'package:mindteck_iot/widgets/site_list_row.dart';

import '../resource/app_resource.dart';

class SiteScreen extends ConsumerStatefulWidget {
  const SiteScreen({super.key});

  @override
  ConsumerState<SiteScreen> createState() => _SiteDManagementState();
}

class _SiteDManagementState extends ConsumerState<SiteScreen> {
  void _reload() {
    ref.invalidate(siteListProvider);
    ref.read(siteListProvider);
  }

  @override
  Widget build(BuildContext context) {
    final deviceData = ref.watch(siteListProvider);

    return Scaffold(
      body: CustomScrollView(
        primary: true,
        slivers: <Widget>[
          SliverAppBar(
            forceElevated: true,
            elevation: 4,
            floating: true,
            pinned: true,
            title: Text(
              AppText.sites,
              style: GoogleFonts.latoTextTheme().titleLarge!.copyWith(
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
                    fontSize: 18,
                  ),
            ),
            iconTheme: IconThemeData(
              color: Colors.white, // Change the color of the back button
            ),
            actions: [],
          ),
          SliverFillRemaining(
            child: deviceData.when(
              data: (data) {
                List<SiteData> siteList = data.map((e) => e).toList();

                if (siteList.isEmpty) {
                  return Container(
                    alignment: Alignment.center,
                    child: NoDataScreen(onRetry: _reload),
                  );
                }
                return ListView.builder(
                  padding: EdgeInsets.zero,
                  itemBuilder: (context, index) {
                    return SiteManagementListRow(
                      sitelist: siteList[index],
                    );
                  },
                  itemCount: siteList.length,
                );
              },
              error: (error, stackTrace) {
                return Center(
                  child: ErrorScreen(
                    errorMessage: error.toString(),
                    onRetry: _reload,
                  ),
                );
              },
              loading: () {
                return Center(
                  child: LoadingAnimationWidget.staggeredDotsWave(
                    color: Colors.blueAccent,
                    size: 70,
                  ),
                );
              },
            ),
          ),
        ],
      ),
    );
  }
}
